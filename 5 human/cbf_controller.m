...
pause(1);
rob_pos = handles.LatestMessage.Pose(8); %extract robot pose
...
while (norm(robot.current_state(1:2)-x_goal)>0.6)
    if (norm(robot.current_state(1:2)-init_goal)<2.2)
        robot.x_goal = x_goal;
    end
    ...
    obs_vec.current_pose = [p{1}(end,:)', p{2}(end,:)',p{3}(end,:)', p{4}(end,:)',p{5}(end,:)'];
    obs_vec.current_r=[0.3; 0.3;0.3;0.3;0.3];
...
%     end
    ...
    real_curr_act31 = [real_curr_act31 ;obs_vec.current_pose(:,3)'];
    real_curr_act41 = [real_curr_act41; obs_vec.current_pose(:,4)'];
    real_curr_act51 = [real_curr_act51 ;obs_vec.current_pose(:,5)'];
    ...
    real_curr_act32 = [real_curr_act32 ;env_map.round_down(10,[handles.LatestMessage.Pose(5).Position.X handles.LatestMessage.Pose(5).Position.Y])];
    real_curr_act42 = [real_curr_act42 ;env_map.round_down(10,[handles.LatestMessage.Pose(6).Position.X handles.LatestMessage.Pose(6).Position.Y])];
    real_curr_act52 = [real_curr_act52 ;env_map.round_down(10,[handles.LatestMessage.Pose(7).Position.X handles.LatestMessage.Pose(7).Position.Y])];
