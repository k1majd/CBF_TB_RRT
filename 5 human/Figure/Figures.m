...
for k=1:length(rrt_obj.graph_h)
    ...
    h_new = rrt_obj.graph_h(k).node(node_sel).hr_min;
    if h_new<0
        h_new=0;
    end
    h_vex=[h_vex h_new];
...
for i=1:56
    if animate
        str = {['Linear Velocity: ',num2str(u_traj(1,i),'%1.3f'),' [m/s],     ', 'Angular Velocity: ',num2str(u_traj(2,i),'%1.3f'),' [rad/s]'] ...
        ['Cost: ',num2str(rrt_obj.graph_h(i).node_select_cost,'%2.3f'),' [1/m^2],    ','               Safety Measure: ',num2str(h_vex(i),'%2.3f'),' [m^2]']};
        anot=annotation('textbox',[.13 .55 .7 0.45],'String',str,'EdgeColor','none','fontsize',20);
    end
    G = rrt_obj.graph_h(i);
    axis([G.node(1).pose(1)-3.5 G.node(1).pose(1)+4 G.node(1).pose(2)-2 G.node(1).pose(2)+3])
    %plot robot
...
%     end
    graph_plot_blue = {};
    graph_plot_blue{1} = plot(G.edge(1).xr(1,:),G.edge(1).xr(2,:),'b','LineStyle','--','LineWidth',2);
    if G.edge_indx>1
        for j=2:length(G.edge)
            if ~(sum(G.edge(j).xr(1,:)>1.66) && sum(G.edge(j).xr(2,:)>1.5))
                graph_plot_blue{j} = plot(G.edge(j).xr(1,:),G.edge(j).xr(2,:),'b','LineStyle','--','LineWidth',2);
            end
    ...
    drawnow
    if i==42
        title_str = ['Time: ',num2str(t(i)),' [s]'];
        tit = title(title_str,'Interpreter','latex','fontsize',20);
    end
    ...
    if i~=length(rrt_obj.graph_h)
        ...
        for pp=1:length(graph_plot_blue)
            set(graph_plot_blue{pp},'Visible','off')
        end
        ...
        graph_plot = {};
        graph_plot{1} = plot(G.edge(1).xr(1,:),G.edge(1).xr(2,:),'Color',lightBlue);
        if G.edge_indx>1
        for j=2:length(G.edge)
            if ~(sum(G.edge(j).xr(1,:)>1.66) && sum(G.edge(j).xr(2,:)>1.5))
                graph_plot{j} = plot(G.edge(j).xr(1,:),G.edge(j).xr(2,:),'Color',lightBlue,'LineWidth',1.5);
...
legend('$min_{c/d} h^{c/d}(\cdot)$ [$m^2$]','fontsize',20,'Interpreter','latex')
